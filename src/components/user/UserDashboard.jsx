    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <Card3D depth={20} onClick={() => setActiveUserTab('incident-report')}>
          <div className="cursor-pointer group bg-white rounded-3xl p-6 border border-slate-200 hover:border-brand-amber-500 shadow-card-soft hover:shadow-2xl transition-all duration-300 flex flex-col justify-between h-full">
            <div>
              <div className="w-12 h-12 rounded-2xl bg-amber-50 text-brand-amber-600 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                <AlertOctagon className="w-6 h-6" />
              </div>
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-bold text-base text-slate-900 group-hover:text-brand-amber-600 transition-colors">
                  Incident Reporting
                </h3>
            ...
            <div className="mt-4 flex items-center text-xs font-bold text-brand-amber-600 group-hover:translate-x-1 transition-transform">
        ...
        <Card3D depth={20} onClick={() => setActiveUserTab('donations')}>
          <div className="cursor-pointer group bg-white rounded-3xl p-6 border border-slate-200 hover:border-brand-teal-600 shadow-card-soft hover:shadow-2xl transition-all duration-300 flex flex-col justify-between h-full">
            <div>
              <div className="w-12 h-12 rounded-2xl bg-teal-50 text-brand-teal-800 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                <Heart className="w-6 h-6" />
              </div>
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-bold text-base text-slate-900 group-hover:text-brand-teal-800 transition-colors">
                  80G Donations
                </h3>
            ...
            <div className="mt-4 flex items-center text-xs font-bold text-brand-teal-800 group-hover:translate-x-1 transition-transform">
        ...
        <Card3D depth={20} onClick={() => setActiveUserTab('adoptions')}>
          <div className="cursor-pointer group bg-white rounded-3xl p-6 border border-slate-200 hover:border-brand-mint-500 shadow-card-soft hover:shadow-2xl transition-all duration-300 flex flex-col justify-between h-full">
            <div>
              <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-brand-mint-600 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                <Baby className="w-6 h-6" />
              </div>
              <div className="flex items-center justify-between mb-1">
                <h3 className="font-bold text-base text-slate-900 group-hover:text-brand-mint-700 transition-colors">
                  Adoption Portal
                </h3>
                <span className="text-[10px] bg-brand-mint-100 text-brand-teal-900 font-bold px-2 py-0.5 rounded-full">
                  Admin Vetted
                </span>
              </div>
              <p className="text-xs text-slate-600 leading-relaxed mt-1">
            ...
            <div className="mt-4 flex items-center text-xs font-bold text-brand-mint-700 group-hover:translate-x-1 transition-transform">
        ...
        <Card3D depth={20} onClick={() => setActiveUserTab('volunteers')}>
          <div className="cursor-pointer group bg-white rounded-3xl p-6 border border-slate-200 hover:border-indigo-500 shadow-card-soft hover:shadow-2xl transition-all duration-300 flex flex-col justify-between h-full">
            <div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-card-soft">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-slate-500">Real-time distress rescue lifecycle across India</p>
              </div>
              <button
                onClick={() => setActiveUserTab('incident-report')}
                className="text-xs font-bold text-brand-teal-800 hover:text-brand-teal-900 bg-brand-teal-50 px-3 py-1.5 rounded-xl border border-brand-teal-200"
              >
                + Report New
              </button>
            </div>
            ...
            <div className="space-y-4">
              {incidents.slice(0, 3).map(inc => (
                <div
                  key={inc.id}
                  className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 hover:bg-white hover:shadow-md transition-all flex flex-col sm:flex-row gap-4 items-start sm:items-center"
                >
                  <img
                    src={inc.photo}
                    alt={inc.title}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {inc.status === 'Resolved' && inc.resolutionNotes && (
                      <p className="text-[11px] text-emerald-800 font-medium bg-emerald-50 p-1.5 rounded-lg mt-1.5 line-clamp-1">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-card-soft">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500">Food, medicines & emergency items</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {requirements.slice(0, 2).map(req => {
                ...
                return (
                  <div key={req.id} className="p-4 rounded-2xl bg-amber-50/50 border border-amber-200/70">
                    <div className="flex items-center justify-between text-[11px] font-bold text-amber-900 mb-1">
                      <span className="bg-amber-100 px-2 py-0.5 rounded-md">{req.urgency}</span>
                      <span>{percent}% Funded</span>
                    </div>
                    <h4 className="text-xs font-bold text-slate-900 mt-1 line-clamp-1">{req.title}</h4>
                    <p className="text-[11px] text-slate-500">{req.ngoName}</p>
                    ...
                    <div className="flex items-center justify-between mt-3 text-[11px]">
                      <span className="font-bold text-slate-700">₹{req.raisedValue.toLocaleString()} of ₹{req.targetValue.toLocaleString()}</span>
                      <button
                        onClick={() => setActiveUserTab('donations')}
                        className="font-bold text-brand-amber-700 hover:text-brand-amber-900 underline"
                      >
                        Contribute &rarr;
                      </button>
                    </div>
